...
3. **对话** — 每个角色以自己的方式跟你开口——这是对话的一半，等你接下一句
4. **会饮** — 各角色的回应编织为一篇会饮记录，师傅们围坐讨论你的问题
...
```
/crescent-cave [选角] [结构] [档位] 你的问题
```

三个维度正交，自由组合，均可省略（省略用默认值，**加粗**项）。

| 维度 | 选项 | 说明 |
|---|---|---|
| **选角** | **问道** · 活水 · 游方 · 乱入 | AI匹配 · 角色自己站出来 · 有方向也有意外 · 纯抽签全看命 |
| **呈现** | **会饮** · 流觞 | 并行出稿后编织为对话录 · 串行接力，对话感最真也最慢 |
| **档位** | 一言 · 短论 · **深谈** · 著书 | 2-3句 · 200-400字 · 800-1200字 · 不限 |

角色各自的半对话作为中间产物保留，同时编织为会饮记录（对话录体）一起呈现。每个角色标注来源：`[AI选]` `[自荐]` `[随机]`。
